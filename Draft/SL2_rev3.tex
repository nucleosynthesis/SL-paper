...
% A normal distribution has a support on $\mathbf{R}$, while quantities like event yields are defined on $\mathbf{R}^+$. Having a nuisance parameter with a normal distribution can therefore give inconsistencies such as a negative yield. Truncating the normal distribution to keep only the positive values does not solve this paradox,  because the posterior can be improper in such case (see \textit{e.g.} \cite{Obj_Bayes}).~\footnote{ The fact that the posterior is improper is tied to  the fact that the truncated normal density  does not vanish at the point of truncation. } 
%%This is a problem both conceptually and concretely, when marginalising or profiling the likelihood. 
%The paradox actually occurs because the Gaussian limit of the CLT loses information about the support. Using the Gaussian limit is simply too rough an approximation. Instead, to have an asymmetric support such as $\mathbf{R}^+$,  the distribution must be asymmetric. Therefore the skew must be taken into account. %%% SK, 31/01/19 %%%  , which solves the negative yield paradox. 
...
%
% the Gaussian limit of the CLT loses information about the support. Using the Gaussian limit is simply too rough an approximation. Instead, to have an asymmetric support such as $\mathbf{R}^+$,  the distribution must be asymmetric. Typical distributions including the asymmetry will be shown in  Fig.~\ref{fig:approxs}. 
...
%Finally, although systematic uncertainties on the \textit{signal} are easily accounted for (as explained in section~\ref{se:EL_SL}), when uncertainties that are correlated with the background are taken into account, the $b,c,\rho$ parameters  become dependent on the parameter of interest ${\bm \alpha}$. 
...
%thus the uncertainties do not depend on ${\bm \alpha}$.
...
\begin{figure}[t]  \centering
  ...
  \caption{2D distributions of $\hat{n}_{b,I}$ against $\hat{n}_{b,J}$ for the LHC-like 
    ...
    experimental pseudo-search %(black points) 
