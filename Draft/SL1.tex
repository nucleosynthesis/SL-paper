...
Figure~\ref{fig:SLConvergence} shows the RMS of the simplified likelihood coefficients for the three bins, $i=4,50$ and $86$ 
relative to the values determined from 100,000 pseudo-datasets.
...
\caption{RMS of the simplfied likelihood coefficients relative to the mean coefficient value determined from 100,000 pseudo-datasets
    for $a_{I}$ (left), $b_{I}$ (center), and $c_{I}$ (right). The distributions are shown for $I=4$ (black line), $I=62$ (red line) and $I=86$ (blue line).
...
\caption{2D distributions of $\hat{n}_{I}$ against $\hat{n}_{J}$ for
  ...
  $I=62,J=86$ (bottom right) in pseudo-datasets generated from the experimental
  toy search (black points) as described in the text. The background heat map is
  generated from 100,000 samples from the true toy model, the dashed pink
  countours from the linear SL form, and the solid light-blue contours from the
  quadratic SL form. In the pair of high-statistics bins in the top-left plot,
  clear agreement is seen between the linear and quadratic SL forms; in the
  top-right, deviations start to appear, and in the low-statistics bin~$J=86$ of
  the bottom plot the asymmetry is seen to become very significant, and the
  linear SL form has a significant probability density fraction in the
  negative-yield region.}
\end{figure}

% \fixme{ANDY: discuss truncation and positivity: any-negative fraction of
%   $\sim 11.6\%$ estimated for this toy model from 500k samples from the linear
%   SL form, reduced to exactly zero with the quadratic form. Note the quadratic
%   form's better ability to model the max-likelihood point in the true
%   % $\{\hat{n}\}$
%   $\hat{\bm{n}}$ distribution, but at the cost of the very sudden cutoff and
%   corresponding contour distortion. Systematic relaxation of this cutoff to more closely
%   model the true pdf would require evaluation of higher-order coefficients
%   (and/or off-diagonal skew terms) and hence higher moments of the experimental
%   distributions. These shapes are propagated through the composite Poisson
%   component of the likelihood (see the benchmarks against exclusion limits), but
%   the elimination of negative yields --- a problem for Poisson evaluation,
%   currently solved by \emph{ad hoc} and uncontrolled means --- is a major
%   advantage of the SL method.}

\fixme{MATTHEW, SYLVAIN: Plots showing moment-extraction convergence}
...
\fixme{ANDY: small extension to existing practice. Representation in HepData and data formats.}
