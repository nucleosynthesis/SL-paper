...
\renewcommand{\arraystretch}{1.1} % space between table rows
...
an elegant method to obtain the simplified likelihood parameters  is the following. From the estimators of the event yields $\hat n_I$, one can evaluate the three first moments of the distribution and deduce the parameters of the simplified likelihood directly from these moments.  What is needed is the mean, the covariance matrix and the diagonal skew of $\hat n_I$.
 The centered moments are noted $m_n$.  Using the definition $ n_I= a_I+b_I \theta_I+c_I \theta_I^2 $, we have the relations
  \be
  m_{1,I}={\bf E}[\hat n_I]= a_I+c_I\,,
  \ee
%\be
%  {\bf E}[(\hat n_I - {\bf E}[\hat n_I])^2 ]= n_{0,I}^2(\Delta_I^2+2\gamma_I^2)\,,
%  \ee
  \be
  m_{2,IJ}={\bf E}[(\hat n_I - {\bf E}[\hat n_I])(\hat n_J - {\bf E}[\hat n_J]) ]= b_I b_J \rho_{IJ}+2 c_I c_J\rho_{IJ}^2\,,
  \ee
   \be
  m_{3,I}={\bf E}[(\hat n_I - {\bf E}[\hat n_I])^3 ]=   6 b_I^2 c_I+8 c_I^3\,,
  \ee
where ${\bf E}[\cdot]$ denotes the expectation value.
Inverting these relations -- taking care of picking the relevant solutions to quadratic and cubic equations -- gives directly the parameters of the simplified likelihood.  We find
\be
c_I=-{\rm sign} (m_{3,I})\sqrt{2 m_{2,II}} \cos\left(\frac{4\pi}{3}+\frac{1}{3}\arctan\left(\sqrt{8 \frac{m^3_{2,II}}{m^2_{3,I}}-1}\right)  \right)
\ee
\be
...
a_I=m_{1,I}- c_I
\ee
These formulas apply if the condition $8 m_{2,II}^3\geq m_{3,I}^2$ is satisfied. This limit is approached when the asymmetry becomes large. Near this limit, the approximation typically tends to become inaccurate because higher order terms $O(\theta^3)$  would need to be included in the expressions of the  simplified likelihood Eq.~\eqref{eq:SL_master}. In practice however,  this would happen for fairly extreme skewness of the nuisance parameters, and it turns out that the simplified likelihood  framework up to quadratic order is sufficient  in most of concrete applications.


This method will be used in the examples shown in the rest of the paper. This means that if one is provided with the moments $m_{1}$ and $m_{3}$ for each bin and the covariance matrix $m_{2,IJ}$, the simplified likelihood parameters are completely defined. Moreover, in the case where the nuisance parameters are affecting only the background rate Eq.~\eqref{eq:SL_LHC}, this computation has to be realized once and the resulting likelihood can be used for any kind of signal by appropritate substitution of $n_{s}(\alpha)$.
...
The reader interested only in the practical aspects of the simplified likelihood framework can safely skip it. In subsection~\eqref{se:skew_CLT} we lay down a  result about the next-to-leading term of the central-limit theorem. Then in subsection~\eqref{se:analytic} we demonstrate Eq.~\eqref{eq:SL_master} and give the analytical expressions of the simplified likelihood parameters as a function of the elementary uncertainties.
...
In the following subsection, we describe a toy search for new physics, inspired by those performed at the LHC, and derive the simplified likelihood parameters for it.
...
to derive the expected background contribution. The uncertainties due to this, indicated by the blue band, is uncorrelated between the different bins. Additionally, there are two uncertainties which modify the `shape' of
...
to this as the `nominal' value since it is the value attained when the elementary nuisance parameters are equal to their expectation values $\underline{\delta}=0$.} number of events due to a particular process in a given bin ($I$) in Eqn~\ref{se:EL_SL} is denoted,

\begin{equation}
 n_{I}({\bm \delta}) := f_{I}({\bm \delta}) N({\bm \delta}),
\end{equation}
...
\begin{figure}[hbt!]
\begin{center}
\includegraphics[width=0.32\textwidth]{figures/distribution_4.pdf}
\includegraphics[width=0.32\textwidth]{figures/distribution_50.pdf}
\includegraphics[width=0.32\textwidth]{figures/distribution_86.pdf}
...
The distributions of $\hat{n}_{I}$ for the two cases (quadratic in green and linear in red) with $\hat{\theta}_{I}\sim\mathcal{N}(0,1)$ are shown.\textbf{TO BE UPDATED WITH FINAL TOY MODEL}}
...
is much smaller when $n^{0}_{I}$ is small, as in the tails of the distributions (eg bin $86$), compared to the linear form.
 ...
 This work has been initiated at the \textit{LHC Chapter II: The Run for New Physics} workshop held at IIP
