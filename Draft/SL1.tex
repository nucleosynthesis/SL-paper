...
As the event counts in each bin (\textit{i.e.} each piece of the distribution) are independent from each other, the simplified likelihood framework is applied to these kinds of analyses (see also Refs.~\cite{Fichet:2016gvx,SL_note}) by identifying the $P$ independant measurements as event counts $o_{I}$ and defining the probabilities $Pr$ of Eqn.~\ref{eq:SL_LHC} as Poisson probabilities for each bin $I$,
\be
...
%
\begin{equation}
 n_{I}(\bm{\delta}) = N^{0}\cdot \prod_{j}(1+K_{j})^{\delta_{j}} \cdot f^{0}_{I} \cdot\frac{1}{F(\bm{\delta})} \prod_{j} p_{Ij}(\delta_{j}) \cdot \prod_{j} (1+\epsilon_{Ij}\delta_{j}).
...
\begin{align}
  ...
  \sum_I^P \left[ o_{I} \ln \left(\mu n_{s,I} + n_{I}(\theta_{I}) \right) - \left( \mu n_{s,I} + n_{I}(\theta_{I}) \right) - o_{I}! \right]
  - \frac{1}{2} \bm{\theta}^\mathrm{T} \bm{\rho}^{-1} \bm{\theta} - \frac{P}{2} \ln 2\pi \\
  %
  \Rightarrow \qquad
  \frac{\mathrm{d}\ln L}{\mathrm{d}\mu} &= \sum_I^P \left( \frac{o_I}{n_I(\bm{\alpha}, \theta_I)} - 1 \right) \cdot s_I(\bm{\alpha}) \\
  %
  \frac{\mathrm{d}\ln L}{\mathrm{d}\theta_{\!A}} &= \left( \frac{o_A}{n_A(\bm{\alpha}, \theta_{\!A})} - 1 \right) \cdot \big( b_A + 2 c_A \theta_{\!A} \big) - \sum_I^P \rho_{\mspace{-1mu}AI}^{-1} \theta_I ~,
\label{eq:SL_LHC}
\end{align}
where $n_{I}(\theta_{I}) = a_{I}+b_{I}\theta_{I}+c_{I}\theta_{I}^{2}$.
