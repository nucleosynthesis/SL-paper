...
\begin{align}
  ...
  & \quad \approx \prod_{I=1}^P \mathrm{Pr}\Big( \hat{n}_I\,\Big|\,a_{I}(\bm \alpha)+b_I(\bm \alpha)\theta_I+c_I(\bm \alpha)\theta_I^2  \Big) \cdot
...
The deformed Gaussian obtained when keeping the skew into account does not seem to have in general an analytical PDF. However, by using the large $N$ expansion, we are able to develop a  trick to express the CLT at next-to-leading order in a very simple way. 
...
Equation~\eqref{eq:ZskewCLT}  readily gives the most basic CLT at next-to-leading order when assuming $Z=N^{-1/2}\sum_{j=1}^N \delta_j$, where the $\delta_j$ are  independent identically distributed centred nuisance parameters  of variance $\sigma^2$ and third moment $\gamma$. The trick applies similarly to the Lyapunov CLT, \ie\ when the $\delta_j$ are not identical, in which case one has defined $\sigma^2=N^{-1}\sum_{j=1}^N \sigma_j^2$, $\gamma=N^{-1}\sum_{j=1}^N \gamma_j$,
...
When $\gamma_i\rightarrow 0$  one recovers the expressions obtained in Ref.~\cite{Fichet:2016gvx}.\footnote{For simplicity we show here the expressions assuming $c_I\ll b_I$, as it is sufficient in the scope of the proof. For sizeable $c_I$, one should instead solve the system  
%shown in Eqs.~\eqref{eq:moments1}-\eqref{eq:moments2}, \eqref{eq:solutions1}-\eqref{eq:solutions2}.  
of Eqs.~\eqref{eq:moments1}---\eqref{eq:solutions2}}
...
As all the elementary uncertainties have in principle different shape and magnitudes we apply Lyapunov's CLT \cite{Billingsley}. We can for instance use Lyapunov's condition on the third moment, and the theorem read as follows. If 
\be
...
The normal approximation for a Poisson and a log-normal distribution. Black curves show the true distributions, red curves show the approximate distributions. \textcolor{blue}{[SK: it is hard to see what is what in this plot; a journal won't accept this. Please do the lines for $\sigma=0.1$, $0.3$ and $0.45$ in three different colours, with the true distributions as solid lines and the approx ones as dashed ones.]}
}
\end{figure}
...
and it is clearly of advantage if their effect can re-evaluated without having to re-analyse the whole data (which could only be done by collaboration insiders).\footnote{Such combination of theoretical uncertainties  has been done in \cite{Arbey:2016kqi}  for the Higgs production and decay rates and can be implemented in a Higgs SL.}
...
As already mentioned in Section~\ref{se:EL_SL}, the dominant systematic uncertainties relevant in searches for new physics are those related to the background processes. Indeed, any misestimation of the background could result in an erroneous conclusion regarding the presence (or absence) of a signal.
There are a number of different ways in which an experimentalist may assess the effect of a given systematic uncertainty, but generally, these effects are parameterized using knowledge of the variations observed in a given process under variations of some underlying parameter of the simulation model, theory, detector resolution, etc. Estimates of the contribution from background processes are obtained either from simulation or through  data-driven methods. Often such methods are accompanied by associated  uncertainties which reduce the ability to make precise statements about the expectation from the background.
These systematic uncertainties are important for new physics searches. 
\textcolor{blue}{[SK: I don't understand the purpose of the previous two sentences]}
In the following section, we describe a pseudo-search for new physics, inspired by those performed at the LHC, and derive the SL parameters for it.
...
to this as the `nominal' value since it is the value attained when the elementary nuisance parameters are equal to their expectation values $\underline{\delta}=0$.} number of background events, due to a particular process, in a given bin ($I$) in %Eqn.~\ref{se:EL_SL} 
...
\begin{figure}[t]
  ...
  \caption{2D distributions of $\hat{n}_{b,I}$ against $\hat{n}_{b,J}$ for the LHC-like 
    ...
    symmetric (red histograms) and asymmetric (green histograms) SL approximations.
    In the pair of
...
from the linear SL form, \eg\ for likelihood marginalisation, requires that the
...
setting negative-rate samples to zero or an infinitesimal value: the 
...
likelihood and the SL. In addition, the result obtained using only the linear terms of the simplified is shown. As expected, the
...
We performed a study of the rate of convergence of the SL coefficients by calculating them using different numbers of pseudo-datasets. 
The coefficients are calculated for three bins, $I=4,62$ and $86$, using independent sets of a fixed number of pseudo-datasets. The root mean square (RMS) 
...
We hope our method will be adopted by both the experimental and theory communities for a better documentation and re-interpretation of the LHC results. Or: If systematically adopted by the experimental and theory communities, the SL has the potential to considerably improve both the documentation and the re-interpretation of the LHC results.
