...
We present an efficient method to compute the parameters of the simplified likelihood  from Monte Carlo simulations. The approach is validated using a realistic LHC-like analysis, and the limits of the approximation are explored.
 The simplified likelihood data can be conveniently released in the  HepData error source format and automatically built from it, making this framework a convenient %ideal
...
as a function of the variance and the skew  of each elementary nuisance parameter (see Sec.~\ref{se:analytic}). However, often the elementary uncertainties and the event yields are already coded in a Monte Carlo generator. In such case, %instead of using Eqs.~\eqref{eq:n_comb}- \eqref{eq:gam_comb},
...
An important aspect of the simplified likelihood is that it is very flexible, because it does not need to be applied to the whole set of systematic uncertainties.  When applied to a subset of systematic uncertainties, the only requirement is that the subset be in the conditions of satisfying the central limit theorem conditions as described in Sec.~\ref{se:SL_theory}. 
...
For example the theoretical uncertainties can be improved later on, hence members of the community should be able to change them
 with no loss of consistency. Such combination of theoretical uncertainties  has been done in \cite{Arbey:2016kqi}  for the Higgs production and decay rates and can be implemented in a simplified likelihood. 
Another reason to single out a nuisance parameter  from the combination (as shown in Eq.~\eqref{eq:SL_variants1})
can  be if it has a large asymmetric distribution that one wants to take into account with no approximation. 
In order to take profit from the  versatility of the simplified likelihood, an equally  versatile format is needed to release the simplified likelihood data. This is the topic of next subsection.  

Finally, if systematic uncertainties on the \textit{signal} are taken into account, the $b,c,\rho$ parameters  become dependent on the parameter of interest ${\bm \alpha}$. While there is no conceptual difference with the case with respect to having only background systematics, there are important practical differences. Numerical evaluations  become heavier  because the parameters of the simplified likelihood -- especially $\rho_{IJ}(\alpha)$ which requires a matrix inversion -- have to be evaluated for each value of ${\bm \alpha}$. Presentation of the simplified likelihood data becomes also more evolved.
...
Also, using the elementary uncertainties maybe more convenient when one wishes to include the systematic uncertainties on the signal, \textit{i.e.}  $\alpha$-dependent $b,c,\rho$. Since these systematics are not crucial for new physics searches we do not take this aspect into account here. 
}
...
% to improve and normalize it in the context of the simplified likelihood framework,
 Let us fix some terminology.  It is convenient to refer to the data in terms of the order of the moment from which they originate. We will use the term ``$n$-th order data'' to refer to information  coming from a moment of order $n$.  $n$ will go only up to $3$. Second order data includes covariance matrix, correlation matrix an diagonal uncertainties -- that can be given either in a relative or absolute parametrization. There is the same kind of freedom for third order data but this does not need to be discussed here. In addition to the moments of the combined systematic uncertainties, this terminology will also apply to the observed central values and statistical uncertainties usually presented by the experiments.
...
Let us review the current formats of presentation of likelihood data. There is no problem of format with the  presentation of first order data and no third order data are usually given. Regarding second order data there is unfortunately no standard representation currently established.
 A review of the second order data in HepData and
on the experiments' analysis websites reveals a mixture of
presentation styles:
%
\begin{itemize}
\item  \textit{Table format:}  2D histograms of either covariance or correlation matrices. This has the
  ...
  first order dataset   it is impossible for computer
  codes to unambiguously construct the relevant likelihood. In the case of the presentation of correlation matrix,    the diagonal variances must be provided with the first order dataset.
  
\item \textit{Error source format:}
A vector of labeled $\pm$ terms associated to
  each element of the first order dataset. The correlations between the error sources is indicated via the labels,  
  (e.g.~a   ''{\tt stat}'' label to be a purely diagonal contribution, a ``{\tt lumi}'' label to be
  100\% correlated across all bins, and all other labeled uncertainties treated
  as orthogonal).
   The correlation or covariance matrices can be constructed using \textbf{[]}.   
  This format presents the second order data  in the form of 
``effective''  elementary uncertainties. 
...
In the case of table format, in addition to the observed central values and statistical uncertainties usually released, extra HepData tables
can encode the $m_{1,I}$, $m_{2,IJ}$, $m_{3,I}$ moments describing the combined nuisance parameters.
However the HepData table headers have to be augmented to
express relationships between tables, i.e.~identifying the moment data tables associated with a first-order dataset in a standardised fashion. While the format is conceptually straightforward, introducing the semantic description of the tables is at present highly unpractical. We will rather prefer the error source format for which the semantic aspect is not an issue. 




In the error source format, the $m_{1,I}$, $m_{2,IJ}$, $m_{3,I}$  moments are \textit{all} encoded in the form of labeled vectors. The $m_{2,IJ}$ matrix is reconstructed via a sum of the form 
\be
m_{2,IJ}= \sum {a_{I,i}a_{J,i}} \, 
\ee
...
\begin{figure}[t]
  ...
  \caption{2D distributions of $\hat{n}_{b,I}$ against $\hat{n}_{b,J}$ for the toy
    ...
    model, the dashed red countours from the linear SL form, and the solid
...
\begin{figure}[t]
  ...
  \caption{Value of $t_{\mu}$ as a function of $\mu$ for the toy search assuming the experimental likelihood (black solid line) and simplified likelihood retaining (green dashed line) or not (red dashed line) the contribution from the quatratic term. The horizontal lines drawn at $t_{\mu}=1$ and $3.86$ represent the
...
The test-statistic can also be constructed for the experimental likelihood $L(\mu,\bm{delta})\pi(\bm{\delta})$, where the same subsitution as in~\ref{eq:muscale} is applied,
...
Figure~\ref{fig:tmucompare} shows a comparison of the value of $t_{\mu}$ as a function of $\mu$ for the toy search between the full (experimental)
...
agreement between the full and simplified likelihood is greatly improved when including the quadratic term.


A horizontal line is drawn at the value of
