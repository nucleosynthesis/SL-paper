...
We present an efficient method to compute the parameters of the simplified likelihood  from Monte Carlo simulations. The approach is validated using a realistic LHC-like toy search, and the limits of the approximation are explored. 
 The simplified likelihood data can be easily released in   HepData tables and automatically built from them, making this framework a convenient %ideal
...
The most natural way to release the data needed to construct the simplified likelihood is in the form of the first three moments of the PDF of the combined nuisance parameters. These moments are the  $m_{1,I}$, $m_{2,IJ}$, $m_{3,I}$ already defined in Eqs.~\eqref{eq:moments1}-\eqref{eq:moments2}, where $m_{3,I}$ is the diagonal part of the third-rank tensor $m_{3,IJK}$.  Evaluating these moments via MC toys is straightforward for the experiments. However their presentation needs to be discussed in great details, taking into account  the available tools and the current practices. This is the purpose of this section. 
...
Let us review the current formats of presentation of likelihood data. There is no problem of format with the  presentation of first order data and no third order data are usually presented. Regarding second order data there is unfortunately no standard representation currently established. 
 At present a review of the second order data in HepData and
...
\item 2D histograms of either covariance or correlation matrices. This has the
  ...
  first order data   it is impossible for computer
  codes to unambiguously construct the relevant likelihood. In the case of the presentation of correlation matrix, the   the diagonal variances must be provided with the first order data.
...
\textbf{[This case is supposed to be about diagonalized covariances but in fact it's not clear to me]}  
...
The first two of these  presentation styles may be readily extended for automated data
handling and are thus appropriate to release simplified likelihood data.
In the first case, in addition to the observed central values and statistical uncertainties usually released, extra HepData tables 
can encode the $m_{1,I}$, $m_{2,IJ}$, $m_{3,I}$ moments describing the combined nuisance parameters. 
The HepData table headers may be augmented to
express relationships between tables, i.e.~identifying the moment data tables associated with a primary dataset in a standardised fashion. 

\textbf{[I stopped here]}
And in the second case, the SL covariance matrices may be represented
...
may also be available between the bins of different distributions/datasets. 
% For
...
\begin{align}
  ...
  %
  \Rightarrow \qquad
  \frac{\mathrm{d}\ln L_{\rm{S}}}{\mathrm{d}\mu} &= \sum_I^P \left( \frac{n^{\rm{obs}}_I}{\mu n_{s,I} + n_{b,I}(\theta_{I})} - 1 \right) \cdot \mu n_{s,I} \\
  %
  \frac{\mathrm{d}\ln L_{\rm{S}}}{\mathrm{d}\theta_{\!A}} &= \left( \frac{n^{\rm{obs}}_A}{\mu n_{s,A} + n_{b,A}(\theta_{\!A}) } - 1 \right) \cdot \big( b_A + 2 c_A \theta_{\!A} \big) - \sum_I^P \rho_{\mspace{-1mu}AI}^{-1} \, \theta_I ~,
\label{eq:SL_LHC}
\end{align}
%
