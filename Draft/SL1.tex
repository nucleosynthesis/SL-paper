...
\begin{align}
  ...
  & \approx \prod_{I=1}^P \mathrm{Pr}\Big( \hat{n}_I\,\Big|\,a_{I}(\bm \alpha)+b_I(\bm \alpha)\theta_I+c_I(\bm \alpha)\theta_I^2  \Big) \cdot
    \frac{ \mathrm{e}^{{\textstyle-\frac{1}{2}\bm \theta^{\rm T} \bm{\rho}^{-1}(\bm \alpha) \bm \theta}}}{\sqrt{(2\pi)^P }}  \equiv L_{\rm{SL}}(\bm{\alpha},\bm{\theta})\,, \label{eq:SL_master}
\end{align}
...
\begin{equation}
L_{\rm{SL}}^{\rm{max}}(\mu) = \rm{max}_{\theta_{I}}\left\{ L_{\rm{SL}}(\mu,\bm{\theta}) \right\}.
\end{equation}
The test-statistic $t_{\mu}$ is then defined using the ratio,

\begin{equation}
t_{\mu} = -2\ln \frac{L_{\rm{SL}}^{\rm{max}}(\mu)}{L_{\rm{SL}}^{\rm{max}}},
\end{equation}
where $L_{\rm{SL}}^{\rm{max}}$ denotes the maximum value of $L_{\rm{SL}}^{\rm{max}}(\mu)$ for any value of $\mu$\footnote{The precise definition of the test-statistic used
...
%
\begin{align}
  \ln\! \big( L_{\rm{SL}}(\mu,\bm{\theta} )\pi(\bm{\theta}) \big) &=
  ...
  %
  \Rightarrow \qquad
  \frac{\mathrm{d}\ln L_{\rm{SL}}}{\mathrm{d}\mu} &= \sum_I^P \left( \frac{n^{\rm{obs}}_I}{\mu n_{s,I} + n_{b,I}(\theta_{I})} - 1 \right) \cdot \mu n_{s,I} \\
  %
  \frac{\mathrm{d}\ln L_{\rm{SL}}}{\mathrm{d}\theta_{\!A}} &= \left( \frac{n^{\rm{obs}}_A}{\mu n_{s,A} + n_{b,A}(\theta_{\!A}) } - 1 \right) \cdot \big( b_A + 2 c_A \theta_{\!A} \big) - \sum_I^P \rho_{\mspace{-1mu}AI}^{-1} \, \theta_I ~,
\label{eq:SL_LHC}
\end{align}
%
