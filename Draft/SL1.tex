...
$n_{b,I}(\hat{\bm{\delta}})$ for each dataset according to the Eqn~\ref{eqn:expt_param}. The black points show the projection of these toy datasets onto each bin index. Instead, the green line shows the distribution of $\hat{n}_{b,I}$ when $\hat{n}_{b,I} = a_{I}+b_{I}\hat{\theta}_{I}+c_{I}\hat{\theta}_{I}^{2}$ and $\hat{\theta}_{I}\sim\mathcal{N}(0,1)$. The simplified likelihood coefficients
...
assuming a linear approximation ($\hat{n}_{b,I} = m_{1,I}+\hat{\theta}_{I}\sqrt{m_{2,II}}$), which assumes the third moment is negligible, is shown by the red line.
...
the likelihood attains it maximum for a given set of $n^{\rm{obs}_I}$.

\begin{equation}
L^{\rm{max}}(\mu) = \rm{max}_{\theta_{I}}\left\{ L(\mu,\bm{\theta}) \right\}.
\end{equation}
The test-statistic $t_{\mu}$ is then defined using the ratio,

\begin{equation}
t_{\mu} = -2\ln \frac{L^{\rm{max}}(\mu)}{L^{\rm{max}}},
\end{equation}
where $L^{\rm{max}}$ denotes the maximum value of $L_{\rm{max}}(\mu)$ for any value of $\mu$\footnote{The precise definition of the test-statistic used
as searches at the LHC and the procedures used to determing limits are slightly different to that presented here and are detailed in Ref.~\cite{CMS-NOTE-2011-005}.}. Similarly, likelihood ratios are also used for quantifying some excess in the case of the discovery
...
\begin{figure}%[hbt!]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/testtoy-tmuscan}
  \caption{Value of $t_{\mu}$ as a function of $\mu$ for the toy search assuming the full likelihood (black solid line) and simplified likelihood retaining (green dashed line) or not (red dashed line) the contribution from the quatratic term. The horizontal lines drawn at $t_{\mu}=1$ and $3.86$ represent the
...
\begin{align}
  ...
  %
  \Rightarrow \qquad
  \frac{\mathrm{d}\ln L}{\mathrm{d}\mu} &= \sum_I^P \left( \frac{n^{\rm{obs}}_I}{\mu n_{s,I} + n_{b,I}(\theta_{I})} - 1 \right) \cdot \mu n_{s,I} \\
  %
  \frac{\mathrm{d}\ln L}{\mathrm{d}\theta_{\!A}} &= \left( \frac{n^{\rm{obs}}_A}{\mu n_{s,A} + n_{b,A}(\theta_{\!A}) } - 1 \right) \cdot \big( b_A + 2 c_A \theta_{\!A} \big) - \sum_I^P \rho_{\mspace{-1mu}AI}^{-1} \, \theta_I ~,
\label{eq:SL_LHC}
\end{align}
%
